...
def fetch_papers_for_ndjson(
    cited_threshold: int = 25, 
    seed_dois: Optional[List[str]] = None,
    top_n: Optional[int] = None
) -> List[Tuple]:
    """
    Fetch papers for NDJSON export based on citation threshold, seed list, or top N by citations.
    
    For small graphs (<10k nodes), includes all papers unless top_n is specified.
    For large graphs, filters by citation threshold or returns top N by citations.
    
    Args:
        cited_threshold: Minimum citation count for papers (ignored if top_n is set or for small graphs)
        seed_dois: Optional list of DOIs to include regardless of citations
        top_n: If set, return top N papers by citation count (overrides cited_threshold)
        
    Returns:
        List of tuples (doi, title, cluster, cited_count, refs_count, x, y, fncr, year)
    """
    run_id = get_active_run_id()
    if run_id is None:
        return []
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            # First check graph size
            cur.execute("SELECT COUNT(*) FROM papers WHERE run_id = %s", (run_id,))
            total_nodes = cur.fetchone()[0]
            
            # If top_n is specified, always use it (order by citation count)
            if top_n is not None and top_n > 0:
                if seed_dois:
                    # Include seed DOIs plus top N by citations
                    cur.execute(
                        """
                        SELECT doi, title, cluster, cited_count, references_count, x, y, fncr, year
                        FROM papers
                        WHERE run_id = %s
                        ORDER BY 
                            CASE WHEN lower(doi) = ANY(%s) THEN 0 ELSE 1 END,
                            cited_count DESC NULLS LAST
                        LIMIT %s
                        """,
                        (run_id, seed_dois, top_n),
                    )
                else:
                    cur.execute(
                        """
            ...
            # For small graphs, include all papers
            ...
            # For large graphs, use citation threshold
            elif seed_dois:
                cur.execute(
                    """
                    SELECT doi, title, cluster, cited_count, references_count, x, y, fncr, year
...
def fetch_edges_involving_dois(dois: List[str]) -> List[Tuple[str, str]]:
    """
    Fetch all edges where at least ONE end is in the DOI list.
    Used for bridge recommendations to find papers that connect to the collection.
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                SELECT src, dst
                FROM edges
                WHERE run_id = %s AND (src = ANY(%s) OR dst = ANY(%s))
                LIMIT 10000
            """,
                (run_id, dois, dois),
            )
...
def fetch_nodes_in_bbox(
    ...
    """
    Fetch nodes within a bounding box for LOD (level of detail) loading.
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT doi, title, x, y, cluster, cited_count, references_count, year, fncr
                FROM papers
                WHERE run_id = %s
                ...
                LIMIT %s
                """,
                (run_id, x_min, x_max, y_min, y_max, min_citations, limit),
            )
            
            nodes = []
            for row in cur.fetchall():
                doi, title, x, y, cluster, cited, refs, year, fncr = row
                nodes.append({
                    "doi": doi,
...
def fetch_paper_positions(dois: List[str]) -> List[Dict]:
    """
    Fetch x, y positions for a list of DOIs.
    
    Args:
        dois: List of DOIs to fetch positions for
        
    Returns:
        List of dicts with doi, x, y
    """
    run_id = get_active_run_id()
    if run_id is None or not dois:
        return []
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                SELECT doi, x, y
                FROM papers
                WHERE run_id = %s AND doi = ANY(%s) AND x IS NOT NULL AND y IS NOT NULL
                """,
                (run_id, dois),
            )
            
            return [{"doi": doi, "x": float(x), "y": float(y)} for doi, x, y in cur.fetchall()]


def fetch_papers_in_radius(
    center_x: float,
    center_y: float,
    radius: float,
    min_citations: int = 0,
    ...
    """
    Fetch papers within a circular radius of a center point.
    ...
    # Use bounding box for efficient filtering, then calculate actual distance
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT doi, title, x, y, cluster, cited_count, year
                FROM papers
                WHERE run_id = %s
                  AND x BETWEEN %s AND %s
                  AND y BETWEEN %s AND %s
            ...
            papers = []
            for doi, title, x, y, cluster, cited, year in cur.fetchall():
...
def fetch_papers_by_dois(dois: List[str]) -> List[Dict]:
    """
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT doi, title, cluster, cited_count, year, x, y
                FROM papers
                WHERE run_id = %s AND doi = ANY(%s)
                """,
                (run_id, dois),
            )
            
            papers = []
            for doi, title, cluster, cited, year, x, y in cur.fetchall():
                papers.append({
                    "doi": doi,
                    "title": title or "Untitled",
                    "cluster": cluster,
